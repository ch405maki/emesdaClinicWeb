          ...
          <!-- PDF template content -->
            <div id="pdf-template">
                ...
                <div class="border-t border-gray-200 px-4 py-2">
                  ...
                  <div>
                    <p class="text-sm font-medium text-gray-600">
                      Occlusion: <span class="text-gray-800">{{ diagnostic.occlusion || 'No result yet' }}</span>
                    </p>
                    <p class="text-sm font-medium text-gray-600">
                      Periodontal Condition: <span class="text-gray-800">{{ diagnostic.condition || 'No result yet' }}</span>
                    </p>
                    <p class="text-sm font-medium text-gray-600">
                      Oral Hygiene: <span class="text-gray-800">{{ diagnostic.hygiene || 'No result yet' }}</span>
                    </p>
                    <p class="text-sm font-medium text-gray-600">
                      Denture Upper: <span class="text-gray-800">{{ diagnostic.denture_upper || 'No result yet' }}</span>
                    </p>
                    <p class="text-sm font-medium text-gray-600">
                      Denture Lower: <span class="text-gray-800">{{ diagnostic.denture_lower || 'No result yet' }}</span>
                    </p>
                    <p class="text-sm font-medium text-gray-600">
                      Abnormalities: <span class="text-gray-800">{{ diagnostic.abnormalities || 'No result yet' }}</span>
                    </p>
                    <p class="text-sm font-medium text-gray-600">
                      General Condition: <span class="text-gray-800">{{ diagnostic.general_condition || 'No result yet' }}</span>
                    </p>
                    <p class="text-sm font-medium text-gray-600">
                      Physician: <span class="text-gray-800">{{ diagnostic.physician || 'No result yet' }}</span>
                    </p>
                    <p class="text-sm font-medium text-gray-600">
                      Nature of Treatment: <span class="text-gray-800">{{ diagnostic.nature_treatment || 'No result yet' }}</span>
                    </p>
                    <p class="text-sm font-medium text-gray-600">
                      Allergies: <span class="text-gray-800">{{ diagnostic.alergies || 'No result yet' }}</span>
                    </p>
                    <p class="text-sm font-medium text-gray-600">
                      Previous History of Bleeding: <span class="text-gray-800">{{ diagnostic.history || 'No result yet' }}</span>
                    </p>
                    <p class="text-sm font-medium text-gray-600">
                      Chronic Ailments: <span class="text-gray-800">{{ diagnostic.ailments || 'No result yet' }}</span>
                    </p>
                    <p class="text-sm font-medium text-gray-600">
                      Blood Pressure: <span class="text-gray-800">{{ diagnostic.bp || 'No result yet' }}</span>
                    </p>
                    <p class="text-sm font-medium text-gray-600">
                      Drugs Being Taken: <span class="text-gray-800">{{ diagnostic.drugs || 'No result yet' }}</span>
                    </p>
                  </div>
                  <!-- Chart -->
                  </div>
                  ...
                  <div class="row">
                    ...
                    <div v-for="tooth in teeth_upper" :key="tooth" class="flex items-center mb-2">
                      <input 
                        type="checkbox" 
                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                        :value="tooth" 
                        v-model="dentalChart['Upper Operation']"
                      />
