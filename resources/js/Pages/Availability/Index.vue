<template>
                    ...
                    <div class="flex flex-col md:flex-row w-full justify-center">
                        ...
                        <div class="w-full md:w-2/3 p-1">
                            <v-table class="shadow-sm rounded-lg">
                                ...
                                <tbody>
                                    <tr v-for="entry in availability" :key="entry.id">
                                        <td>{{ entry.dentist.name }}</td>
                                        <td>{{ formatDate(entry.date) }}</td>
                                        <td>{{ formatTime(entry.start_time) }} - TO - {{ formatTime(entry.end_time) }}</td>
                                        <td class="">
                                            <button @click="deleteItem(entry.id)" class="p-2 rounded-full  group transition-all duration-500  flex item-center">
                        ...
                        <div class="w-full md:w-1/3 p-2 flex justify-start">
                            <form @submit.prevent="saveAvailability" class="w-full bg-white p-5 rounded-lg shadow-md">
                                ...
                                <!-- Dentist Selection -->
                                <div class="mb-4">
                                    <label for="dentist_id" class="block text-sm font-medium text-gray-700">Select Dentist:</label>
                                    <select v-model="form.dentist_id" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-300 focus:border-blue-500">
  ...
  // Get props including the list of dentists
  ...
  // Use useForm directly to manage form state
  const form = useForm({
    dentist_id: '',
    date: '',
    start_time: '',
...
// Function to format date to "September 29, 2024"
...
const formatTime = (timeString) => {
    ...
    if (timeParts.length < 2) {
        return 'Invalid Time'; // Handle invalid time input
    }
    
    // Ensure to only use hours and minutes
    const hours = timeParts[0];
    const minutes = timeParts[1];

    // Create a Date object using hours and minutes
    const date = new Date(`1970-01-01T${hours}:${minutes}:00`);
    
    if (isNaN(date.getTime())) { // Check if the time is valid
        return 'Invalid Time'; // Or return a placeholder like 'N/A'
    }
